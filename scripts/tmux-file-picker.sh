...
main() {
	...
	if [[ ${#search_dirs[@]} -eq 1 ]]; then
		...
		selected_files_str=$(cd "${search_dirs[0]}" && $fd_cmd $fd_flags | fzf --multi --reverse --freeze-right=1 --preview "$preview_cmd" || true)
	else
